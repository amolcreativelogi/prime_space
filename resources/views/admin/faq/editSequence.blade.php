...
<div class="panel panel-default">
 ...
 <div class="panel-heading">
    ...
    <div class="panel-body">  
     <form action="<?php echo url('admin/faqs/saveFaqsCategorySequece'); ?>" method="post" enctype="multipart/form-data" id="form-user" class="form-horizontal">
         ...
         <div class="table-responsive">
                ...
                <table id="example" class="table table-striped">
                  ...
                  <?php if(!empty($getFaqCategories)){?>
                    ...
                    @foreach($getFaqCategories as $categories)
                      <tr>
                        <td><?= $categories ->category_name?></td>
                        <td>
                        <select name="sequence[<?= $categories ->category_id?>]">
                          @foreach($sequenceArr as $sequence)
                            <option <?php echo ($sequence == $categories->sequence)?"selected":""; ?> value="<?=$sequence;?>"><?= $sequence;?>
                            </option>
                          @endforeach
                        </select>
                        ...
                        <td><?= ($categories ->status == '0')?"Inactive":"Active"; ?></td>
                      </tr>
                    @endforeach
                    @if(count($getFaqCategories) > 1)
                    <tr>
                      <td colspan="3">
