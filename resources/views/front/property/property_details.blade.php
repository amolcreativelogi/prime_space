...
@section('content')
    ...
    <div class="site-content">
        <?php if(empty($getPropertyDetails)){
            echo '<h4>No contents to display.</h4>';
        }else{?>
        <div class="single-property">
            ...
            <section class="property-content">
                    ...
                    <div class="row">
                        <div class="col-lg-8 col-md-8 col-sm-12 pc-left">
                            ...
                            <h4><i class="fa fa-map-marker"
                                   aria-hidden="true"></i> <?php echo !isset($getPropertyDetails->location) ? '' : $getPropertyDetails->location;?>
                            </h4>
                            <div class="row firstrow">
                                ...
                                <div class="col-sm-6"><i class="fa fa-server" aria-hidden="true"></i> Number of floor
                                    <strong>: <?php echo ($NOoffloor) ? $NOoffloor->floor : ''; ?> </strong></div>
                                <div class="col-sm-6"><i class="fa fa-list" aria-hidden="true"></i> Total Parking Slots
                                    <strong>: <?php echo ($NOoffloor) ? $NOoffloor->spots : ''; ?></strong></div>
                                <div class="col-sm-12"><i class="fa fa-car" aria-hidden="true"></i> Type of parking
                                    <strong>: <?php echo ($parkingType->parking_type) ? $parkingType->parking_type : ''; ?></strong>
                                </div>
                            </div>
                            <div class="about-property">
                                <h4>About the space</h4>
                                <p>Zip Code
                                    - <?php echo !isset($getPropertyDetails->zip_code) ? '' : $getPropertyDetails->zip_code;?></p>
                                <p><?php echo !isset($getPropertyDetails->description) ? '' : $getPropertyDetails->description;?></p>

                                <?php if($module_id == 3) { ?>
                                <p>Tour Availability
                                    - <?php $tour_availability = !isset($getPropertyDetails->tour_availability) ? '' : $getPropertyDetails->tour_availability;
                                    echo ($tour_availability == 1) ? 'Yes' : 'No';
                                    ?></p>

                                <p>Land Type
                                    - <?php echo !isset($land_type_id->land_type) ? '' : $land_type_id->land_type;
                                    ?></p>

                                <p>Property Size
                                    - <?php echo !isset($unit_type_id->unit_type) ? '' : $unit_type_id->property_size . ' ' . $unit_type_id->unit_type;
                                    ?></p>
                            <?php } ?>
                                ...
                                <div class="row secondrow">
                                  ...
                                  <tbody>
                                      <tr>
                                        ...
                                        <a href="#" data-toggle="modal" class="floor-map" data-target="#floor-map">Floor
                                            Map</a>
                                        <!-- searchModal start -->
                                        <div class="modal fade formModal" id="floor-map" tabindex="-1" role="dialog"
                                                ...
                                                <div class="modal-content">
                                                    ...
                                                    <div class="modal-body">
                                                        <?php if(isset($getPropertyImagesFloorMap->name)) { ?>
                                                          ...
                                                          <?php $ext = pathinfo($getPropertyImagesFloorMap->name, PATHINFO_EXTENSION);

                                                          if($ext == 'pdf'){ ?>

                                                            <iframe src="<?php echo URL::to('/public/images/property-floor-map/' . $getPropertyImagesFloorMap->name . ''); ?>" style="width:100%; height:1000px; " scrolling="no" frameborder="0"></iframe>

                                                         <?php }else { ?>

                                                        <img src="<?php echo URL::to('/public/images/property-floor-map/' . $getPropertyImagesFloorMap->name . ''); ?>"
                                                             width="100">
              <?php }  ?>
                            ...
                            <div class="property-review">
                                ...
                                <div class="rating">
                                    ...
                                    <label style="color: #5500fe" class="full" for="star<?= $i ?>-<?= $i ?>"
                                           title="Awesome - <?= $i ?> stars"></label>
                                    <?php else: ?>
                      ...
                      <label class = "full" for="star1-5" title="Sucks big time - 1 star"></label> -->
                                ...
                                <div class="reviewbox">
                                    <div class="reviewer-img"><img
                                                src="{{ URL::asset('public') }}/assets/front-design/images/user-icon.jpg"
                                                alt=""></div>
                                    <div class="reviewer-info">
                                        <h4>{{$bookingDataRating->firstname}} {{$bookingDataRating->lastname}}</h4>
                                        <span>{{$bookingDataRating->rating_date}}</span>
              ...
              <?php } ?>
                                ...
                                <a href="<?php echo URL::to('pages/cancellation-and-refund-policy'); ?>">Cancellation
                                    Policy</a>
                            </div>
                            <?php } ?>
                            ...
                            <a href="<?php echo URL::to('messages?property_id=' . $getPropertyDetails->property_id . '') ?>"
                               class="cont-host"> contact host</a>
                            <?php }  } else { ?>
                            <a href="#>" data-toggle="modal" class=" loginModal cont-host"> contact host</a>
                            <?php } ?>
                            <br/>
                            <a href='<?php echo URL('/') ?>/bookNow?moduleid=<?php echo Request::get("moduleid") . "&propertyid=" . Request::get("propertyid") . "&duration_type_id=" . Request::get("duration_type_id") . "&location_type_id=" . Request::get("location_type_id") . "&car_type_id=" . Request::get("car_type_id") . "&fromdate=" . Request::get("fromdate") . "&todate=" . Request::get("todate") . "&fromtime=" . Request::get("fromtime") . "&totime=" . Request::get("totime") . "&durationtype=" . Request::get("durationtype")?>'
                               class="cont-host">Book now</a>
                            <hr>
                        </div><!-- pc-right -->
                    </div>
                </div>
