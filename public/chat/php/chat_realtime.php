...
class Chat_realtime {
			...
			if($ke == 'all'){
				...
				while($r = $sql->fetch()){
					...
					if(!in_array($name, $tmp)){
						array_push($tmp, $name);
						$get=$this->dbh->prepare("SELECT status FROM  prk_user_registrations WHERE name=?");
	...
	function get_user($user){
		...
		if(isset($user)){
			$sqlm=$this->dbh->prepare("SELECT firstname as name FROM prk_user_registrationsHide WHERE firstname=?");
		...
		$data = array();
		$sql=$this->dbh->prepare("SELECT * FROM prk_user_registrations");
		$sql->execute();
		while($r = $sql->fetch()){
			$data["all"][] = array(
				'name' => $r['firstname'],
